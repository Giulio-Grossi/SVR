...
sim_model<-function(seed_b, seed_t, seed_e,
  ...
  #### construct a vector of GP coefficients - length(beta)=bands*num_control
  ...
  Sigma <- curr_alpha * (exp(- D_xloc ^ 2 / (2*(curr_rho ^ 2)))) + sp_nugget * diag(1, length(treated_radius))  # covariance matrix of the beta GP 
  ...
  # ---- PART C: Genearting potential outcomes for the treated units ----- #
