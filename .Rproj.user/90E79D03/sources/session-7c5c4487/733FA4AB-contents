wrapper<-function(sim_l, cal_l, cov_l, exp){
res_gp=list()
res_sc=list()
res_sr=list()
res_vr=list()
...
for(i in c(1:length(sim_l))){
  ...
  for (ii in 1:length(sim_l)){
    ...
    # cc[["MGP"]]=postset(aa, cal[[ii]][["MGP"]], tt0[i])
    # cc[["SC"]]=postset(aa, cal[[ii]][["SC"]], tt0[i])
    # cc[["SR"]]=postset(aa, cal[[ii]][["SR"]], tt0[i])
    # cc[["BVR"]]=postset(aa, cal[[ii]][["BVR"]], tt0[i])
  ...
  gp_fin=apply(simplify2array(gp[-c(76:80)]), 1:2, mean, na.rm=T)
