...
model {
  ...
  K1 = gp_exponential_cov(h, w*alpha, rho) + diag_matrix(rep_vector((1-w)*alpha, n_t)) ;
  L = cholesky_decompose(K1);
   for(ii in 1:n_c)
   K_beta[ii] = gp_exponential_cov(h, alpha_b[ii], rho_b);
   for(ii in 1:n_c)
...
generated quantities{
  ...
  K11 = gp_exponential_cov(h, w*alpha, rho)  +  diag_matrix(rep_vector((1-w)*alpha, n_t));
