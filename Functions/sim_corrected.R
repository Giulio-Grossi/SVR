...
##  control_radius <- runif(num_controls, min = 0.05, max = 0.23)
  ...
  death_all[1 : (num_important + 1), ] <- death_prob_control[1]
  death_all[ - c(1 : (num_important + 1)),] <- death_prob_control[2]
  # Entry 1 is the treated area, and 2 - 11 are the control areas.
  for (tt in 2 : time_periods) {
    death_all[1 : (num_important + 1), tt] <- expit(logit(death_all[1, tt - 1]) + rnorm(1, 0, sd = 0.02))
    death_all[(num_important + 2) : (num_controls + 1), tt] <- expit(logit(death_all[num_important + 2, tt - 1]) + rnorm(1, 0, sd = 0.02))
  }
  
  plot(death_all[1, ], type = 'l', ylim = range(death_all))
  lines(death_all[num_important + 2, ], col = 'red')
  
  # Need to do something similar for birth rates.
  birth_all <- matrix(NA, nrow = length(Wsplit), ncol = time_periods)
  birth_all[1 : (num_important + 1), ] <- birth_rate_control[1]
  birth_all[- c(1 : (num_important + 1)), ] <- birth_rate_control[1]
  for (tt in 2 : time_periods) {
    birth_all[1 : (num_important + 1), tt] <- exp(log(birth_all[1, tt - 1]) + rnorm(1, 0, sd = 0.02))
    birth_all[(num_important + 2) : (num_controls + 1), tt] <- exp(log(birth_all[num_important + 2, tt - 1]) + rnorm(1, 0, sd = 0.02))
  }
  
  plot(birth_all[1, ], type = 'l', ylim = range(birth_all))
  lines(birth_all[num_important + 2, ], col = 'red')
 ...
 # }
  ...
  # Plotting all of them as a function of time:
 # plot(1, type = 'n', ylim = c(- 4, 4), xlim = c(1, time_periods))
 # for (rr in 1 : length(treated_radius)) {
 #   lines(1 : time_periods, (Ytreat[, rr] - mean(Ytreat[1:t0, rr])) / sd(Ytreat[1:t0, rr]), col = 'red')
 # }
 # for (cc in 1 : num_important) {
 #   Sys.sleep(1)
 #   lines(1 : time_periods, (Ycon[, cc] - mean(Ycon[1:t0, cc])) / sd(Ycon[1:t0, cc]), lwd = 0.5)
 # }
 # for (cc in (num_important + 1) : num_controls) {
 #   Sys.sleep(1)
 #   lines(1 : time_periods, (Ycon[, cc] - mean(Ycon[1:t0, cc])) / sd(Ycon[1:t0, cc]), col = 'yellow', lwd = 0.8)
 # }
